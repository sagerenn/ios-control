...
class CiReleaseWorkflowTests(unittest.TestCase):
    ...
    def test_release_build_structure_contains_expected_matrix_and_artifacts(self) -> None:
        ...
        self.assertIn("date -u +%Y%m%d%H%M%S", workflow_text)
        self.assertIn("cargo build --release --locked --target ${{ matrix.target }}", workflow_text)
        self.assertIn("cross build --release --locked --target ${{ matrix.target }}", workflow_text)
        ...
        self.assertIn("--timestamp ${{ steps.timestamp.outputs.value }}", workflow_text)
