...
#[test]
fn inventory_aggregator_merges_live_and_known_rows_on_known_device_id() {
    let snapshot = aggregate_inventory(vec![
        DeviceObservation {
            provider: InventoryEvidenceSource::Bluetooth,
            stable_id: Some("{PHONE-1}".into()),
            known_device_id: Some("known-alice".into()),
            display_name: "Alice iPhone".into(),
            mirror_source_id: None,
            live: true,
        ...
        DeviceObservation {
            provider: InventoryEvidenceSource::Known,
            stable_id: Some("BTHENUM\\DEV_A".into()),
            known_device_id: Some("known-alice".into()),
            display_name: "Alice iPhone".into(),
            mirror_source_id: None,
            live: false,
    ...
    assert_eq!(snapshot.devices.len(), 1);
    assert_eq!(snapshot.devices[0].stable_id.as_deref(), Some("{PHONE-1}"));
}
